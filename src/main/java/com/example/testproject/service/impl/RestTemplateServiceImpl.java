...
public class RestTemplateServiceImpl implements RestTemplateService {
    ...
    public String getAroundHub() {
        URI uri = UriComponentsBuilder //어떤 경로로 요청을 할건지 지정하는 클래스
    ...
    public String getName() {
        URI uri = UriComponentsBuilder
                ...
                .queryParam("name", "Flature") //파라미터 요청
                .encode()
                .build()
                .toUri();
        RestTemplate restTemplate = new RestTemplate();
    ...
    public ResponseEntity<MemberDTO> postDto() {
        ...
        ResponseEntity<MemberDTO> responseEntity = restTemplate.postForEntity(uri, memberDTO,
                MemberDTO.class); //postForEntity는 uri, 요청객체, 반환타입을 인자로 줌
        LOGGER.info("status code : {}", responseEntity.getStatusCode());
        LOGGER.info("body : {}", responseEntity.getBody());
    ...
    public ResponseEntity<MemberDTO> addHeader() {
        ...
        ResponseEntity<MemberDTO> responseEntity = restTemplate.exchange(requestEntity,
                MemberDTO.class); //exchange는 엔티티, 반환타입 넘겨줌
        LOGGER.info("status code : {}", responseEntity.getStatusCode());
        LOGGER.info("body : {}", responseEntity.getBody());
        return responseEntity;
    }
